...
let sample_gaussian bound sigma large_num =
  ...
  in
  let u = Z.(to_float (div (Mirage_crypto_pk.Z_extra.gen large_num) large_num)) in
...
let gen_public_key (priv_key:private_key) =
  ...
  for j = 0 to m - 1 do
    a.(n-1).(j) <- Z.of_int (abs (sample_gaussian bound 10000000000.0 q));
    for i = 0 to n - 2 do
